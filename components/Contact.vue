    ...
    <form class="w-full flex flex-col justify-center items-end mt-5" @submit.prevent="sendForm()">
      <div class="relative w-full flex flex-col justify-center items-end">
        <input
            ...
            v-model="form.name"
            required
            class="relative my-1 w-full lg:w-2/2 p-2 pl-10 text-color-text/60 rounded-lg border-[1px] border-gray-300 focus:outline-none focus:ring-1 focus:ring-text-color-primary focus:border-transparent"
        />
        <span class="w-full lg:w-2/2 absolute inset-y-0 flex items-center pointer-events-none px-2.5">
      ...
      <div class="relative w-full flex flex-col justify-center items-end">
        <input
            ...
            class="relative my-1 w-full lg:w-2/2 p-2 pl-10 text-color-text/60 rounded-lg border-[1px] border-gray-300 focus:outline-none focus:ring-1 focus:ring-text-color-primary focus:border-transparent"
        />
        <span class="w-full lg:w-2/2 absolute inset-y-0 start-50 flex items-center pointer-events-none px-2.5">
      ...
      <div class="relative w-full flex flex-col justify-center items-end">
        <textarea
            ...
            required
            class="relative my-1 w-full lg:w-2/2 p-2 h-42 min-h-50 text-color-text/60 rounded-lg border-[1px] border-gray-300 focus:outline-none focus:ring-1 focus:ring-text-color-primary focus:border-transparent"
        />
      </div>
      ...
      <div class="relative w-full flex flex-col justify-center items-end">
        <button
            ...
            @click="!loading"
            :disabled="loading"
            class="w-full rounded-lg lg:w-2/2 p-2 my-1 text-base font-medium text-center inline-flex items-center justify-center text-color-background shadow-sm bg-color-text/90 hover:bg-color-text/60 relative from-secondary-color via-primary-color to-primary-color transition-all duration-300"
        >
        ...
        <NuxtLink
            @click="goToHome()"
            class="hidden lg:flex cursor-pointer items-center hover:text-color-primary/50 mt-10"
        >
